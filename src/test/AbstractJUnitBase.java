...
//				+ b);
	...
	/**
	 * Setup the test folder path. Dev environment and runtime environment have
	 ...
	 * bin, the we use that for the test If it isn't we add the
	 * the pathAdjustment to the folder path. That way this will work in
	 * development and testing.
	 *
	 * @param pathAdjustment - the project folder to inject into the path. Used when testing from cmd line
	 * @throws IOException on error
	 */
	protected String initFolderPath(String pathAdjustment) throws IOException
	{
		// set defaults
		String filePath = "";
		String targetFolder= "\\";
		...
		// determine if it is eclipse or test harness pathing
		boolean exists = curDir1.exists();
		boolean isDir = curDir1.isDirectory();
		String bin = curDir1.getParentFile().getName();
		boolean isBin = bin.equals("bin");
		
		// adjust and set the folder path
		File srcFolder = null;
		if(exists && isDir && isBin)
		{
			// get the src folder
			File binParent = curDir1.getParentFile().getParentFile();
			File[] fileList = binParent.listFiles();
			srcFolder = findSrcFolder(fileList);
			filePath = srcFolder.getCanonicalPath() + targetFolder;
		}
		else
		{
			...
			filePath = srcFolder.getCanonicalPath() + altTargetFolder;//"\\week08\\FermiGame.java";
		}
	...
	protected String initFilePath(String fileName, String pathAdjustment) throws IOException
	{
		...
		filePath = initFolderPath(pathAdjustment) + fileName;
		return filePath;
	}
